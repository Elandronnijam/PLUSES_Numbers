...
/*
	Задача: Написать программу, которая выводит информацию о введённом числе
	Выводимая информация делится на 5 пунктов:
		П.1: Количество цифр в числе
		П.2: Разбиение числа на разряды
		П.3: Посчет факторала числа
		П.4: Нахождение делителей числа
		П.5: Разложение на простые множители

	Добавить пользователю возможность выбирать интересующую его информацию (номера пунктов)
...
{
	//Не думаю, что ради одного лишь pow стоит подключать cmath...
	int degree = 1;
	while (index > 0)
	{
		degree = degree * 10;
...
{
	/*
Задача: Написать функцию, которая ищен факториал от введенного числа
...
int searching_next_prime_number(int prime)						//Выводит следущее простое число
{
	...
	while (esc != 10000001)
	{
		...
		if (esc >= 10000001)
		{
			std::cout << "Error: esc > " << esc << ", to big number!";
			return prime_copy;
		}
	}
	return prime_copy;
...
int ITEM_1_counting_numbers(int MD, int CP)
{
	if (on_or_off_item(1, CP) == 1)
	{
		...
		int numbers = counting_numbers(MD);							//Количество циферок в числе :)
		if (numbers == 1)
		{
...
Решение:
	1. Ищем в числе максимальную цифру (5), если не наши, искать повторо бесполезно
		...
		Отдельная функция, которая получает число (CHOOSED_POINTS) и номер пункта, а далее:
			...
			ELSE return 0;
		Придумать цикл while или for чтобы:
...
*/
	if (on_or_off_item(5, CP) == 5)
	{
		...
		int prime = 2;												//Простое число, которое собираемся искать
		while (MD > 1)
		{
			int sum = 0;											//Счетчик удачных делений, который символизирует о том, что чило не простое 
																	//(если разделилось еще на что-то кроме себя и 1)
			while (MD % prime == 0)
			{
				MD = MD / prime;
				sum++;
